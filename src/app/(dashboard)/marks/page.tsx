...
import { getCachedClasses, getCachedExams, getCachedExamConfigs, getCachedSections, getCachedSubjects, getCachedSchoolInfo } from "@/lib/cache/master-data-cache";
...
export default function MarksPage() {
    ...
    // ── Data fetching: dropdowns + settings from cache ──
    useEffect(() => {
        let cancelled = false;
        (async () => {
            const [cData, eData, configData, sData] = await Promise.all([
                getCachedClasses(),
                getCachedExams(),
            ...
            setClasses(cData);
            setExams(eData);
            setExamSubjectConfigs(configData);
            if (sData?.detailed_marks) setDetailedMarks(true);
        })();
        return () => {
            cancelled = true;
        };
    ...
    // ── Fetch sections + subjects from cache when class changes ──
    useEffect(() => {
        if (!selectedClass) return;
        let cancelled = false;
        (async () => {
            const [sData, subData] = await Promise.all([
                getCachedSections(selectedClass),
                getCachedSubjects(selectedClass),
            ]);
            if (cancelled) return;
            setSections(sData);
            setSubjects(subData);
     ...
     * When silent = true (auto-save), it saves in the background without button animations or toasts.
     */
    ...
    const handleCellCommit = useCallback(
        (studentId: string, field: "marks" | "theory" | "mcq" | "practical", value: string) => {
            ...
            // Debounced silent auto-save after 600ms
            if (autoSaveTimerRef.current) {
                clearTimeout(autoSaveTimerRef.current);
            }
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        {(saving || autoSaving) && (
                            <span className="text-muted-foreground font-medium text-xs flex items-center gap-1.5">
                                <ArrowsClockwise size={12} className="animate-spin text-primary" />
                                (Saving…)
                            </span>
                        )}
                        {!saving && !autoSaving && hasUnsaved && (
                            <span className="text-amber-500 font-medium text-xs">(Unsaved)</span>
                        )}
                        {!saving && !autoSaving && !hasUnsaved && loaded && students.length > 0 && (
                    ...
                    <Card className="bg-card rounded-2xl border-border shadow-none overflow-hidden">
                        ...
                        <div className="flex items-center justify-end px-5 py-3 border-t bg-muted/20">
                            <Button
                                ...
                                disabled={saving}
                                size="sm"
                                className="border-border text-foreground font-semibold rounded-xl hover:bg-muted transition-all duration-200 h-8 px-4 text-xs bg-transparent border shadow-none"
                            >
                                {saving ? (
                                    <ArrowsClockwise size={14} strokeWidth={1.5} className="mr-1.5 animate-spin text-primary" />
                                ) : (
