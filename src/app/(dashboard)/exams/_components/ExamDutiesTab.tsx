...
export function ExamDutiesTab({ exams }: { exams: { id: string; name: string }[] }) {
    ...
    const fetchDuties = useCallback(async () => {
        ...
        if (!selectedDate || !selectedShift) {
            setDuties([]);
            const { data } = await supabase.from("exam_duties").select("teacher_id").eq("exam_id", selectedExam);
        ...
        try {
            const [currentDutiesRes, allDutiesRes] = await Promise.all([
                supabase.from("exam_duties").select("room_id, teacher_id")
                    ...
                    .eq("start_time", start_time)
                    .eq("end_time", end_time),
                supabase.from("exam_duties").select("teacher_id, exam_date, start_time, end_time").eq("exam_id", selectedExam)
            ]);
            ...
            (allDutiesRes.data || []).forEach(d => {
                const isCurrentShift = d.exam_date === selectedDate && d.start_time === start_time && d.end_time === end_time;
                if (!isCurrentShift) {
...
<body>
    ...
    const handlePrintTotalDutyCounts = () => {
        ...
        const tableRowsHtml = teachers.map((t, index) => {
...
<body>
    ...
    <div class="report-banner">
        <div>
            <div class="report-title">Teacher Exam Duty Summary Report</div>
            <div class="report-exam">Exam: ${selectedExamName}${examDateRangeText ? ` (${examDateRangeText})` : ""}</div>
        </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 110px" class="text-center">Contact</th>
                <th style="width: 85px" class="text-center">Total Duties</th>
                <th class="sig-cell text-center">Signature / Remarks</th>
            </tr>
        </thead>
        <tbody>
            ${tableRowsHtml}
                                                        ...
                                                        {roomDutiesList.map(d => {
                                                            ...
                                                            return (
                                                                <Badge key={d.teacher_id} variant="secondary" className="inline-flex items-center gap-1.5 py-1 pl-2.5 pr-1 rounded-md text-xs font-medium">
                                                                    ...
                                                                    <span 
                                                                        className="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold rounded-full bg-background text-foreground border border-border shadow-xs"
                                                                        title={`${count} total ${count === 1 ? "duty" : "duties"}`}
                                                                    >
                                                                        {count}
                                                                    </span>
                                                                    <button 
