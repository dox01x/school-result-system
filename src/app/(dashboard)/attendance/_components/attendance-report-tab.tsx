...
export function AttendanceReportTab({ filters }: Props) {
    ...
    const studentSummaries = useMemo<StudentSummary[]>(() => {
        ...
        for (const row of rows) {
            ...
            const day = parseInt(row.att_date.split("-")[2], 10);
                        ...
                        <table className="w-full text-xs">
                            ...
                            <tbody>
                                {filtered.map((student, idx) => (
                                    ...
                                    >
                                        <td className="sticky left-0 z-10 bg-inherit py-2.5 px-3 font-mono font-bold text-muted-foreground border-r border-border text-[11px] w-14 min-w-[56px] max-w-[56px]">
                                            {student.roll}
                                        </td>
                                        <td className="sticky left-14 z-10 bg-inherit py-2.5 px-3 font-bold text-foreground truncate min-w-[120px] max-w-[140px] border-r border-border text-[11px]">
                                            {student.name}
                                        </td>
                                        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((d) => {
