...
export default function SettingsPage() {
    ...
    const [paperCheckingRate, setPaperCheckingRate] = useState<string>("0");
    const [savingPrefs, setSavingPrefs] = useState(false);
    const [classRates, setClassRates] = useState<Record<string, string>>({});
    const [classRecheckRates, setClassRecheckRates] = useState<Record<string, string>>({});
        ...
        (async () => {
            try {
                ...
                if (schoolData) {
                    ...
                    // Self-healing: if current_academic_year was missing in DB, persist the resolved one
                    if (!schoolData.current_academic_year && schoolData.id) {
                        await supabase
                            .from("school_info")
                ...
                if (schoolData?.paper_checking_rate !== undefined && schoolData?.paper_checking_rate !== null) {
                    setPaperCheckingRate(String(schoolData.paper_checking_rate));
                }
                ...
                // Fetch other data with fallback
                let loadedClasses: Class[] = [];
                try {
                    const { data: primaryClasses, error: classErr } = await supabase
                        .from("classes")
                        .select("id, name, numeric_value, paper_checking_rate, paper_recheck_rate, created_at")
                        .order("numeric_value");
                    if (!classErr && primaryClasses) {
                        ...
                        const { data: fallbackClasses } = await supabase
                            .from("classes")
                            .select("id, name, numeric_value, created_at")
                            .order("numeric_value");
                ...
                // Initialize class-wise paper checking & rechecking rates
                const initialRates: Record<string, string> = {};
                const initialRecheckRates: Record<string, string> = {};
                loadedClasses.forEach((c) => {
                    initialRates[c.id] = c.paper_checking_rate !== undefined && c.paper_checking_rate !== null ? String(c.paper_checking_rate) : "0";
                    initialRecheckRates[c.id] = c.paper_recheck_rate !== undefined && c.paper_recheck_rate !== null ? String(c.paper_recheck_rate) : "0";
                });
                setClassRates(initialRates);
                setClassRecheckRates(initialRecheckRates);
            } catch (err) {
    ...
    const handleSaveClassRates = async () => {
        setSavingClassRates(true);
        try {
            const updatePromises = classes.map((c) => {
                const rateVal = parseFloat(classRates[c.id]) || 0;
                const recheckVal = parseFloat(classRecheckRates[c.id]) || 0;
                return supabase
                    .from("classes")
                    .update({ 
                        paper_checking_rate: rateVal,
            ...
            setClasses((prev) =>
                prev.map((c) => ({
                    ...c,
                    paper_checking_rate: parseFloat(classRates[c.id]) || 0,
                    paper_recheck_rate: parseFloat(classRecheckRates[c.id]) || 0,
                }))
            );
            toast.success("Class-wise paper checking and rechecking rates saved successfully");
        } catch (err) {
        ...
        <div className="space-y-6">
            ...
            <Tabs defaultValue="school" className="space-y-6">
                ...
                <TabsContent value="preferences" className="space-y-4">
                    ...
                    {/* Class-Wise Paper Checking Remuneration Rates */}
                    <Card className="border border-border shadow-none bg-card rounded-xl">
                        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div>
                                <CardTitle className="text-base flex items-center gap-2 font-bold tracking-tight text-foreground">
                                    <SlidersHorizontal size={18} strokeWidth={2} className="text-muted-foreground" />
                                    Class-Wise Paper Checking &amp; Rechecking Rates
                                </CardTitle>
                            ...
                            <div className="flex items-center gap-2">
                                <Button
                                    ...
                                    className="h-10 px-5 rounded-xl text-xs font-bold bg-primary text-primary-foreground shadow-none"
                                >
                        ...
                        <CardContent>
                            {classes.length === 0 ? (
                                <div className="py-6 text-center text-xs text-muted-foreground italic">
                                    No classes found. Classes will appear here once loaded.
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5">
                                    {classes.map((cls) => (
                                        <div key={cls.id} className="p-3.5 rounded-xl bg-muted border border-border/60 space-y-2.5">
                                            <div className="flex items-center justify-between border-b border-border/40 pb-1.5">
                                                <Label className="text-xs font-bold text-foreground">{cls.name}</Label>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <div className="space-y-1">
                                                    <span className="text-[10px] font-semibold text-muted-foreground block truncate">Main Check</span>
                                                    <div className="relative">
                                                        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground font-bold text-[9px]">BDT</span>
                                                        <Input
                                                            type="number"
                                                            min="0"
                                                            step="0.5"
                                                            value={classRates[cls.id] ?? "0"}
                                                            onChange={(e) => setClassRates((prev) => ({ ...prev, [cls.id]: e.target.value }))}
                                                            className="pl-8 h-8 bg-card border-0 shadow-xs text-xs font-mono font-bold rounded-lg text-right"
                                                        />
                                                    </div>
                                                </div>
                                                <div className="space-y-1">
                                                    <span className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 block truncate">Recheck</span>
                                                    <div className="relative">
                                                        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-indigo-500/70 font-bold text-[9px]">BDT</span>
                                                        <Input
                                                            type="number"
                                                            min="0"
                                                            step="0.5"
                                                            value={classRecheckRates[cls.id] ?? "0"}
                                                            onChange={(e) => setClassRecheckRates((prev) => ({ ...prev, [cls.id]: e.target.value }))}
                                                            className="pl-8 h-8 bg-card border-0 shadow-xs text-xs font-mono font-bold rounded-lg text-right text-indigo-600 dark:text-indigo-400"
                                                        />
                                                    </div>
                                                </div>
                                            </div>
