...
export default function RoutinePage() {
    ...
    const handlePrint = () => {
        // Build table HTML from current data (Clean unified cells without colored boxes or accent lines)
        const headerCells = periodSlots
            .map(
                (slot) =>
                    `<th>
                        <div class="p-num">PERIOD ${slot.period}</div>
                        <div class="p-time">${formatTime12(slot.start)} – ${formatTime12(slot.end)}</div>
                    </th>`
            )
            .join("");

        const bodyRows = workingDays
            .map((dayName, dayIndex) => {
                const cells = periodSlots
                    .map((slot) => {
                        const entries = findRoutinesForSlot(dayIndex, slot);
                        if (entries.length === 0) return `<td class="empty"><span class="dash">&ndash;</span></td>`;
                        if (viewMode === "class") {
                            const parts = entries.map((entry) => {
                                const teacher = getTeacher(entry.teacher_id);
                                const subjName = getName(allSubjects, entry.subject_id);
                                return `
                                    <div class="cell-content">
                                        <div class="subj">${subjName}</div>
                                        <div class="tchr-info">
                                            <span class="tchr-name">${teacher?.name || ""}</span>
                                            ${teacher?.phone ? `<span class="tchr-ph">${teacher.phone}</span>` : ""}
                                        </div>
                                    </div>
                                `;
                            });
                            ...
                            const parts = entries.map((entry) => {
                                ...
                                return `
                                    <div class="cell-content">
                                        <div class="subj">${getName(classes, entry.class_id)} (${getName(allSections, entry.section_id)})</div>
                                        <div class="tchr-info">
                                            <span class="tchr-name">${subjName}</span>
                                        </div>
                                    </div>
                                `;
                            });
        ...
        const title =
            viewMode === "class"
                ? `CLASS: ${selectedClassName.toUpperCase()} &bull; SECTION: ${selectedSectionName.toUpperCase()}`
                : `TEACHER: ${getName(teachers, selectedTeacher).toUpperCase()}`;

        const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Class Routine - ${viewMode === "class" ? `${selectedClassName}-${selectedSectionName}` : getName(teachers, selectedTeacher)}</title>
<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<style>
@page { size: A4 landscape; margin: 6mm 8mm; }
* { margin: 0; padding: 0; box-sizing: border-box; }
body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; font-family: 'Plus Jakarta Sans', sans-serif; color: #0f172a; font-size: 11px; line-height: 1.4; background: #fff; padding: 6px 12px; }

.pg { width: 100%; max-width: 1100px; margin: 0 auto; }

/* Institutional Header */
.hdr { text-align: center; margin-bottom: 6px; }
.hdr img { height: 44px; margin: 0 auto 3px; display: block; }
.hdr h1 { font-size: 20px; font-weight: 800; color: #0f172a; letter-spacing: 0.6px; margin: 0; text-transform: uppercase; }
.hdr .ad { font-size: 10px; color: #64748b; margin-top: 2px; font-weight: 500; }

/* Clean Title Ribbon */
.tbar { display: flex; align-items: center; justify-content: space-between; background: #f8fafc !important; border: 1.5px solid #334155; padding: 7px 16px; margin: 8px 0 10px 0; border-radius: 6px; }
.tbar .badge-title { font-size: 12.5px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.8px; color: #0f172a; }
.tbar .badge-scope { font-size: 12px; font-weight: 700; color: #0f172a; letter-spacing: 0.5px; }
.tbar .badge-meta { font-size: 10px; font-weight: 600; color: #475569; }

/* Routine Table Grid with Clear Crisp Borders */
table { width: 100%; border-collapse: collapse; border: 1.5px solid #334155; background: #ffffff; }

thead th { background: #f1f5f9 !important; color: #0f172a; padding: 8px 4px; text-align: center; border: 1px solid #334155; vertical-align: middle; }
thead th.day-th { width: 95px; min-width: 95px; font-size: 11px; font-weight: 800; letter-spacing: 0.8px; text-transform: uppercase; border: 1px solid #334155; }

.p-num { font-size: 10px; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase; color: #0f172a; }
.p-time { font-size: 8.5px; font-weight: 500; color: #475569; margin-top: 1px; }

tbody tr { height: 54px; }
tbody td { padding: 6px 4px; text-align: center; vertical-align: middle; border: 1px solid #334155; background: #ffffff; }

td.day-col { font-weight: 800; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; width: 95px; min-width: 95px; color: #0f172a; background: #f8fafc !important; border: 1px solid #334155; text-align: center; }
td.empty { background: #ffffff !important; color: #94a3b8; font-size: 16px; font-weight: 300; }

/* Direct Cell Content without inner boxes or colored backgrounds */
.cell-content { padding: 2px 0; text-align: center; }
.subj { font-weight: 700; font-size: 11.5px; color: #0f172a; line-height: 1.25; }
.tchr-info { margin-top: 2px; }
.tchr-name { font-size: 9.5px; font-weight: 500; color: #475569; line-height: 1.1; display: block; }
.tchr-ph { font-size: 8px; color: #64748b; display: block; margin-top: 1px; font-family: monospace; }

/* Signature Section */
.sigs { width: 100%; table-layout: fixed; margin-top: 32px; border-collapse: collapse; border: none; }
.sigs td { text-align: center; vertical-align: top; border: none; padding: 0 15px; }
.sigb { width: 150px; margin: 0 auto; border-top: 1.5px solid #334155; padding-top: 4px; font-size: 10px; font-weight: 700; color: #334155; text-transform: uppercase; letter-spacing: 0.5px; }

.footer-note { text-align: center; font-size: 8.5px; color: #94a3b8; margin-top: 14px; font-weight: 500; letter-spacing: 0.4px; }
</style></head><body>
<div class="pg">
...
<p class="ad">${schoolInfo?.address || ""}${schoolInfo?.phone ? " &bull; Phone: " + schoolInfo.phone : ""}${schoolInfo?.email ? " &bull; " + schoolInfo.email : ""}</p>
</div>

<div class="tbar">
<div class="badge-title">Weekly Class Schedule</div>
<div class="badge-scope">${title}</div>
<div class="badge-meta">Issued: ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</div>
</div>

<table>
<thead><tr><th class="day-th">Day</th>${headerCells}</tr></thead>
...
<td><div class="sigb">Academic In-Charge</div></td>
<td><div class="sigb">Principal / Headmaster</div></td>
</tr>
</table>

<div class="footer-note">Computer Generated Official Schedule &bull; Valid Without Manual Signature</div>
</div>
</body></html>`;
