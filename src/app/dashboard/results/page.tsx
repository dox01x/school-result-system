...
export default function ResultsPage() {
    ...
    useEffect(() => {
        (async () => {
            ...
            setExams(eRes.data || []);
            const rawRules = (gRes.data || []) as GradingRule[];
            ...
            setGradingRules(dedupedRules.length > 0 ? dedupedRules : (ALL_DEFAULT_GRADING as unknown as GradingRule[]));
            if (sRes.data) {
                setSchoolInfo(sRes.data);
                if (sRes.data.current_academic_year) {
                    setSelectedAcademicYear(sRes.data.current_academic_year);
                }
            }
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Select value={selectedExam} onValueChange={(v) => { setSelectedExam(v); setGenerated(false); }}>
                            ...
                            <SelectContent>
                                <SelectItem value={FINAL_RESULT_ID}>Final Result (Annual Combined)</SelectItem>
                                {exams.map((e) => (
                                    <SelectItem key={e.id} value={e.id}>
                                        {e.name}
                                        {e.exam_type === "semester" && " (Combined)"}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
