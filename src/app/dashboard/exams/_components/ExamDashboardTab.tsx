...
export function ExamDashboardTab({
    ...
    const loadExamDashboardData = useCallback(async (examId: string) => {
        if (!examId) return;
        setLoading(true);

        const [schedRes, dutyRes, distRes, seatRes] = await Promise.all([
            supabase.from("exam_schedules").select("id, exam_id, class_id, subject_id, exam_date, start_time, end_time").eq("exam_id", examId).order("exam_date").order("start_time"),
            supabase.from("exam_duties").select("id, exam_id, teacher_id, room_id, exam_date, start_time, end_time").eq("exam_id", examId),
            supabase.from("exam_paper_distributions").select("id, exam_id, class_id, section_id, subject_id, teacher_id, total_copies, date_given, date_returned, status").eq("exam_id", examId),
            supabase.from("exam_seat_plans").select("id, exam_id, room_id, class_id, allocated_students").eq("exam_id", examId),
        ]);
    ...
    useEffect(() => {
        if (selectedExamId) {
            loadExamDashboardData(selectedExamId);
        }
    }, [selectedExamId, loadExamDashboardData]);
    ...
    const metrics = useMemo(() => {
        ...
        const totalStudentsSeated = seatPlans.reduce((sum, s) => sum + (s.allocated_students || 0), 0);
        const allocatedRoomsCount = new Set(seatPlans.map(s => s.room_id)).size;
    ...
    const getRoomName = (id: string) => rooms.find(r => r.id === id)?.name || "Hall Room";
